...
{	
	...
	{	
		//PrintToChatAll("", szName, PointsAwarded(flNewTime, flNewTime, getCurrentCountByRun(run)));
	}
	...
	{		
		PrintToChatAll("", szName, PointsAwarded(flPrevMapBest, flOldBestTime, getCurrentCountByRun(run)), PointsAwarded(flPrevMapBest, flNewTime, getCurrentCountByRun(run)));
	}
	
	//If neither are true, it's a new run
	else
	{
		//PrintToChatAll("", szName, PointsAwarded(flPrevMapBest, flNewTime, getCurrentCountByRun(run)));			
	}
}
...
public int PointsAwarded(float map_wr, float map_pb, int map_completions)
{
	...
	//PrintToChat("Starting Points: map_wr=%f, map_pb=%f, fl_mapCompletions=%i, eulers=%f", map_wr, map_pb, fl_mapCompletions, EULERS_NUMBER);
